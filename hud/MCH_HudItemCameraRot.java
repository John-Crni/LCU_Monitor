...
public class MCH_HudItemCameraRot extends MCH_HudItem {
    ...
    public void execute() {
        this.drawCommonGunnerCamera(MCH_HudItemCameraRot.ac, MCH_HudItemCameraRot.ac.camera, MCH_HudItemCameraRot.colorSetting, MCH_HudItemCameraRot.centerX + calc(this.drawPosX), MCH_HudItemCameraRot.centerY + calc(this.drawPosY));
    }
    ...
    private void drawCommonGunnerCamera(Entity ac, MCH_Camera camera, int color, double posX, double posY) {
        if (camera != null) {
            ...
            float f = ac.prevRotationYaw + (ac.rotationYaw - ac.prevRotationYaw) / 2.0F;

            f = camera.prevRotationYaw + (camera.rotationYaw - camera.prevRotationYaw) / 2.0F;
